...
effect_spec:
  ...
  | n = LIDENT { Event (n, []) }
  | n = LIDENT LPAREN a = INT+ RPAREN { let a = a |> List.map fst |> List.map int_of_string in Event (n, a) }
  | TILDE n = LIDENT { Not (n, []) }
  | TILDE n = LIDENT LPAREN a = INT+ RPAREN { let a = a |> List.map fst |> List.map int_of_string in Not (n, a) }
