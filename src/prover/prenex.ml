...
let rec prenex_term (t : term) : term prenex =
  ...
  | Tuple ts -> 
    ...
    pure (Tuple ts')
  | Binop (op, t1, t2) ->
    pure (Binop (op, process_term t1, process_term t2))
  | Unop (op, t) ->
    pure (Unop (op, process_term t))
  | PointsTo (t1, t2) ->
    pure (PointsTo (process_term t1, process_term t2))
  | SepConj (h1, h2) ->
    pure (SepConj (process_term h1, process_term h2))
  | Conj (p1, p2) ->
    pure (Conj (process_term p1, process_term p2))
  | Implies (p1, p2) ->
    pure (Implies (process_term p1, process_term p2))
  | Subsumes (s1, s2) ->
    pure (Subsumes (process_term s1, process_term s2))
  ...
  | Apply (f, ts) ->
    pure (Apply (process_term f, List.map process_term ts))
  ...
  | Bind (s1, b) ->
    ...
    let new_body =
      unbox
        (Mk.bind (box_term pf1.body)
           (bind_var x (box_term pf2.body)))
    in
