...
let rec simpl_term (t : term) : term = t
(*
  match t with
  | TVar _ -> t
  | TUnit -> TUnit
...
(** For simplicity, we only simplifies [Sequence] and [Bind] at the moment. *)
let rec simpl_staged_spec (s : staged_spec) : staged_spec =
  ...
  | Exists b -> Exists b
  | Shift b -> Shift b
  | Reset s -> simpl_staged_spec_cont s CNil
