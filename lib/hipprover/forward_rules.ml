...
include Subst (* why do we include here? *)
open Debug
...
type fvenv = {
  ...
  (* proof obligations generated by defining lambdas with specifications.
     we propagate them out instead of checking them immediately to avoid a cyclic
     dependency with entailment. *)
...
*)
      ...
      *)
      ...
      else List.concat (List.map (fun tail -> List.map (fun hd -> hd @ tail) res) phi2)
  ...
  let res, env =
    ...
    | CReset e ->
      ...
      let f1 =
        ...
        let f2 = shift_reset_reduction [[Reset (f, Var r)]] in (* TODO: separate this into other module *)
