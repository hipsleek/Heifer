...
let simplify_term t : term  = 
  let t = Typedhip.Fill_type.fill_untyped_term t in
  let open Rewriter in
  let op_commutes lhs op_pat rhs : Typedhip.term rewriter =
    make_rewriter Ast_matchers.(bin_op_typed any op_pat lhs rhs) (fun typ op lhs rhs -> Ast_builders.(bin_op op rhs lhs ~typ))
  in
  let op_left_associates t1 op1 t2 op2 t3 =
    make_rewriter Ast_matchers.(bin_op_typed any op1 t1 (bin_op_typed any op2 t2 t3)) 
    (fun typ op1 t1 typ2 op2 t2 t3 -> Ast_builders.(bin_op ~typ:typ2 op2 (bin_op ~typ op1 t1 t2) t3))
  in
  let three_way_commute t1 op1 t2 op2 t3 =
    make_rewriter Ast_matchers.(bin_op_typed any op1 (bin_op_typed any op2 t1 t2) t3)
    (fun typ op1 typ2 op2 t1 t2 t3 -> Ast_builders.(bin_op ~typ op2 (bin_op ~typ:typ2 op1 t1 t3) t2)) in
  let rewrites = Ast_matchers.{default_rewriters with
    term_rewriters
    =
      [
      (* these rewrite rules sort any sum/difference into the form (((t1 op t2) op t3) ... op tn)
         where all the constants are moved to the left, where constant folding can take care of them *)
      (* we use left associativity because our parser reads plus/minus as left associative;
         rewriting to be right associative is possible but takes a lot of steps *)
      op_left_associates any (capture (one_of [plus; minus])) any (capture (one_of [plus; minus])) any;
      op_commutes (capture (var nothing)) (capture (one_of [plus; minus])) (capture (constant (num nothing)));
      three_way_commute any (capture (one_of [plus; minus]))
        (capture (var nothing)) (capture (one_of [plus; minus])) (capture (constant (num nothing)));

      (* the original code has a rewrite rule ((a + 2^x) + b) + 2^x => (a + 2^(x + 1)) + b *)
      (* im not sure how to elegantly represent that in this framework *)
      
      (* constant folding rewriters *)
      make_rewriter Ast_matchers.(bin_op plus (constant (num any)) (constant (num any)))
        (fun a b -> Ast_builders.(num (a + b)));

      make_rewriter Ast_matchers.(bin_op minus (constant (num any)) (constant (num any)))
        (fun a b -> Ast_builders.(num (a - b)));

      make_rewriter (bin_op sconcat (constant tstr) (constant tstr)) 
        (fun s1 s2 -> Ast_builders.tstr (s1 ^ s2));

      make_rewriter (bin_op ttimes (constant (num any)) (constant (num any))) 
        (fun s1 s2 -> Ast_builders.num (s1 * s2));

      make_rewriter (bin_op tdiv (constant (num any)) (constant (num any))) 
        (fun s1 s2 -> Ast_builders.(num (s1 / s2)))

    ]
  } in
  rewrite_term rewrites t |> Typedhip.Untypehip.untype_term

let simplify_heap h : kappa =
  let h = Typedhip.Fill_type.fill_untyped_kappa h in
  let open Rewriter in
  let rewrites = Ast_matchers.{default_rewriters with
    kappa_rewriters = [
      make_rewriter (one_of [sep_conj emp any; sep_conj any emp]) (fun k -> k);
    ]
  } in
  rewrite_kappa rewrites h |> Typedhip.Untypehip.untype_kappa
...
let simplify_pure (p : pi) : pi =
  let p = Typedhip.Fill_type.fill_untyped_pi p in
  let open Rewriter in
  let rewrites = Ast_matchers.{default_rewriters with
    pi_rewriters = [
      (* distributive law for Not and EQ*)
      make_rewriter (pnot (atom eq any (constant ttrue)))
      (fun t -> Atomic (EQ, t, {term_desc = Const TTrue; term_type = Bool}));

      (* constant folding for And and Or *)
      make_rewriter (one_of [pand any ptrue; pand ptrue any;
        por any pfalse; por pfalse any])
      (fun t -> t);

      (* equality folding for EQ *)
      make_rewriter (atom eq any any |> combine |> filter ~f:(fun (t1, t2) -> t1 = t2))
      (fun _ -> True)
    ]
  } in
  let result = rewrite_pi rewrites p |> Typedhip.Untypehip.untype_pi in
  debug ~at:5 ~title:"simplify_pure" "%s\n==>\n%s" (Pretty_typed.string_of_pi p) (string_of_pi result);
  result
