...
let parse_spec_attribute (attr : Parsetree.attribute) : Hipcore.Hiptypes.staged_spec option =
  ...
  | { attr_name = {txt = attr_name; _};
      attr_payload = PStr [{pstr_desc = Pstr_eval ({pexp_desc = Pexp_constant {pconst_desc = Pconst_string (annotation, _, _); _}; _}, _); _}];
      _} when String.equal attr_name spec_attribute_name -> Some (parse_staged_spec annotation)
  | _ -> None
