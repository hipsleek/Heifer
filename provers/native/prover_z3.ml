...
let rec z3_sort_of_typ ctx typ =
  ...
  memo begin fun z3_ctx typ ->
    ...
    | TConstr (type_name, args) ->
        let type_def = try Globals.decl_of_type_name type_name
          with
            | Not_found -> raise (Invalid_argument (Printf.sprintf "definition for type constructor %s not found" type_name))
        in
        let param_values = List.combine type_def.typ_params args in
        begin match type_def.typ_kind with
    ...
    | List_int -> z3_sort_of_typ z3_ctx (TConstr ("list", [Int]))
